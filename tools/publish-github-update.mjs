...
import { buildGitHubReleasePlan, validateUpdateProgression } from './update/github-release-plan.mjs';
...
const feedApiPath = `repos/${repository}/contents/channels/${channel}.json`;
...
if (existingFeed.status === 0) {
  ...
  if (!/\b404\b|Not Found/i.test(detail)) fail(`could not inspect existing channel feed: ${detail.trim()}`);
}
...
if (
  ...
  || nextFeed?.asset?.release_tag !== `update-${manifest.update_sequence}`
  || nextFeed?.asset?.name !== path.basename(zipPath)
) {
  fail('signed update feed does not match manifest, repository, tag, and payload');
}
