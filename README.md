...
**1.1.0** · seq **22** · [`moonhyun-cheol/myagent`](https://github.com/moonhyun-cheol/myagent)

Windows 온프렘 코딩 에이전트. 화면은 `ui/workspace` 하나, 시작점은 `MYAgent.exe`.

## 직원 PC (설치)

GitHub 릴리스 **[v1.1.0](https://github.com/moonhyun-cheol/myagent/releases/tag/v1.1.0)** 의 **`MYAgent-v*-install.zip`만** 받습니다.
...
서명이 맞고 `update_sequence`가 **지금보다 크면** 확인창 → `MYAgent.Updater.exe`가 델타 zip을 적용합니다. SemVer가 아니라 **seq 숫자**가 기준입니다. 지금 피드는 seq **22**입니다. 첫 설치는 릴리스의 **install zip**입니다.
