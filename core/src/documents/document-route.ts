  ...
  try {
    ...
    if (workspaceRoot) {
      ...
      if (method === 'GET' && suffix === '') {
        const owned = projectDocuments.list(workspaceRoot).map((d) => ({ ...d, readOnly: false }));
        const shared = projectDocuments.listSharedFor(session).filter((s) => !owned.some((o) => o.id === s.id));
      ...
      if (method === 'GET' && projectMatch) {
        try {
          return sendJson(res, 200, { ...projectDocuments.get(workspaceRoot, projectMatch[1]), readOnly: false });
        } catch (error) {
          if (error instanceof DocumentError && error.status === 404) {
            return sendJson(res, 200, projectDocuments.getShared(session, projectMatch[1]));
          }
          throw error;
        }
      }
      const projectOp = /^\/([a-f0-9-]{36})\/(share|attachments|bundle)$/.exec(suffix);
      if (projectOp) {
        const [, id, action] = projectOp;
        const document = projectDocuments.get(workspaceRoot, id);
        const linkedAssets = [...new Set([...document.markdown.matchAll(/\/attachments\/([a-f0-9-]{36})(?=[?\s)#"']|$)/g)].map((m) => m[1]))];
        const missing = () => linkedAssets.filter((asset) => !projectDocuments.hasAsset(id, asset));
        if (method === 'GET' && action === 'bundle') {
          if (missing().length) throw new DocumentError(409, '본문의 첨부 사본이 아직 없습니다. 첨부 사본 보관 후 내보내세요.');
          const bundle = projectDocuments.bundle(workspaceRoot, id);
        ...
        let opBody;
        try {
          opBody = JSON.parse(Buffer.concat(chunks).toString('utf8'));
        } catch {
          throw new DocumentError(400, '잘못된 JSON입니다.');
        }
        if (!opBody || typeof opBody !== 'object') throw new DocumentError(400, '요청 본문이 필요합니다.');
        if (action === 'share' && opBody.remove !== true && missing().length) throw new DocumentError(409, '먼저 본문에 연결된 첨부 사본을 보관하세요. 원본 챗 삭제 후에도 첨부를 유지하기 위한 조건입니다.');
        if (action === 'share') {
          if (typeof opBody.targetSession !== 'string' || !sessionExists(opBody.targetSession) || (opBody.remove !== undefined && typeof opBody.remove !== 'boolean')) throw new DocumentError(400, '대상 챗/연결 해제 값이 잘못되었습니다.');
          projectDocuments.share(workspaceRoot, id, opBody.targetSession, opBody.remove === true);
        } else if (action === 'attachments') {
          if (typeof opBody.attachmentId !== 'string' || !/^[a-f0-9-]{36}$/.test(opBody.attachmentId)) throw new DocumentError(400, '첨부 ID가 필요합니다.');
          const asset = context.attachment?.(session, opBody.attachmentId);
          if (!asset) throw new DocumentError(404, '현재 챗의 첨부가 필요합니다.');
          projectDocuments.addAsset(workspaceRoot, id, asset);
    ...
    if (method === 'GET' && suffix === '') {
      ...
      const merged = [...new Map(documents.map((d) => [d.id, { ...d, readOnly: !store.ownedBy(scopeFor(d.id), d.id) }])).values()];
      const projectShared = (context.projectDocuments ?? getProjectDocumentStore()).listSharedFor(session)
        .filter((s) => !merged.some((m) => m.id === s.id));
      return sendJson(res, 200, {
        documents: [...merged, ...projectShared],
        root: store.root,
    ...
    if (method === 'GET' && match && !match[2]) {
      try {
        return sendJson(res, 200, { ...store.get(scopeFor(match[1]), match[1]), readOnly: !store.ownedBy(scopeFor(match[1]), match[1]) });
      } catch (error) {
        if (error instanceof DocumentError && error.status === 404) {
          return sendJson(res, 200, (context.projectDocuments ?? getProjectDocumentStore()).getShared(session, match[1]));
        }
        throw error;
      }
    }
    if (method === 'GET' && match) return sendJson(res, 200, { versions: store.versions(scopeFor(match[1]), match[1]) });
