...
- When an **auto-detected target** block is present, treat that tool as confirmed — do NOT ask the user to pick a tool unless they named two conflicting tools. Only ask if no auto-detect block exists and the target is truly ambiguous.
...
Identify the tool and route accordingly. Read full templates from the **Prompt Templates Reference** section below only for the category you need.
...
Read Prompt Templates Reference (below) Template J for the full reference editing template.
...
Read Prompt Templates Reference (below) Template K for the full ComfyUI template.
...
Read Prompt Templates Reference (below) Template L for the full Prompt Decompiler template.
...
| File | Read When |
|------|-----------|
| the **Prompt Templates Reference** section below | You need the full template structure for any tool category |
| the **Credit-Killing Patterns Reference** section below | User pastes a bad prompt to fix, or you need the complete 35-pattern reference |
