...
export interface DocumentRecord { id: string; title: string; markdown: string; revision: number; hash: string; notes: DocumentNote[]; updatedAt: string; source?: 'project'; path?: string; root?: string; readOnly?: boolean }
export type DocumentSummary = Pick<DocumentRecord, 'id' | 'title' | 'revision' | 'updatedAt' | 'source' | 'path' | 'readOnly'>;
