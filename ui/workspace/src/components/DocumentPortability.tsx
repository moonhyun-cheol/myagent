...
type Options={project:string|null;projectRoot?:boolean;sessions:{id:string;title:string}[];attachments:{id:string;name:string}[]};
export function DocumentPortability({session,id,revision,disabled,readOnly,projectRoot,onChanged}:{session:string;id:string;revision:number;disabled:boolean;readOnly:boolean;projectRoot?:boolean;onChanged:()=>void}) {
  ...
  return <details><summary onClick={()=>{if(!options)void run(async()=>{setOptions(await documentApi<Options>(session));});}}>공유·이동·묶음 내보내기</summary>
    ...
    {!projectRoot&&<><button disabled={disabled||busy||readOnly||!options.project} onClick={()=>{if(window.confirm('문서 소유권을 현재 챗의 프로젝트로 이동합니다. 이후 이 챗을 삭제해도 문서는 유지됩니다. 계속할까요?'))void run(()=>post('move',{project:options.project,revision}));}}>현재 프로젝트로 이동</button>
    ...
    <button disabled={disabled||busy||readOnly} onClick={()=>void run(bundle)}>참조·첨부 묶음 다운로드</button><p role="status">{status}</p>
  </details>;
}
