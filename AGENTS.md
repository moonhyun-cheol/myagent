...
- `manifest.json` — product version `1.1.0`. Public label is `MY Agent {version} (update {N})`. Clients follow monotonic `update_sequence` (now 23), not SemVer. Bump sequence on every signed zip; bump SemVer only for user-facing meaning (patch/minor/major). GitHub update titles come from `formatGitHubReleaseTitle`.
...
- **Launcher live update (R-617):** WorkKitLauncher polls **only** `channels/launcher-stable.json` (`my-agent-launcher-feed/v1`, `kind: work-kit-launcher`, tag `launcher-update-{N}`, zip `WorkKitLauncher-*-update-{N}.zip`). Same RSA-PSS key + R-614 hosts. No idle gate. Apply is `--apply-update` (does not use `MYAgent.Updater`, does not stop MY Agent). `npm run publish:launcher-update`.
- **Companion work-environment prompt (R-618):** MY Agent polls `GET /system/work-environment/pending` (launcher feed + org catalog). Same idle gate as R-605. Yes → catalog `POST /profiles/catalog/refresh` + spawn `WorkKitLauncher.exe --companion-update`. Does not use `MYAgent.Updater`.
...
| Work kit launcher UI | `ui/work-kit-launcher` | Mode manager at `/launcher/` (WorkKitLauncher.exe) |
| Shell (WPF) | `shell/CqrPa.Shell` | Custom title chrome (`MainWindow.xaml`) |
| Work kit launcher shell | `shell/WorkKitLauncher` | WPF + WebView2 → `/launcher/` |
| Core API | `core/src` | Routes in `core/src/routes/dispatch.ts` |
| Rulebook (권위) | `../RULEBOOK/MY_CUSTOM_CODEX` (`.rulebook-link.yml`) | **GitHub 미포함.** 로컬 전용. 편집은 RULEBOOK만 |
| Rulebook (빌드 스텁) | `rulebook/docs/generated/` | publish용 복사본 (gitignore). `npm run build`가 RULEBOOK에서 합침 |

## Hard rules

1. **Do not invent file state.** `read_file` (or live facts) before asserting Title/WindowStyle/confirm/API paths.
2. **UI target map** — title bar ≠ ChatPane. See `core/config/defaults/ui-facts.json` (`title_bar` vs `chat_pane`).
...
5. **Single UI** — MY Agent product UI is `ui/workspace` at `/`. WorkKitLauncher is a **separate WinExe** (`ui/work-kit-launcher` at `/launcher/`). Do not put work-kit picker back in Settings. `ui/web` and `/legacy` were removed.
...
- Structure / OWUI TEXT: ADR-004 + RULEBOOK `docs/specs/technical/03-core-logic.md` §3.7 (로컬)
...
- Rulebook 합본: `../RULEBOOK/MY_CUSTOM_CODEX/docs/generated/RULEBOOK_MY_CUSTOM_CODEX_MAIN_v*.md`
