...
- `manifest.json` — product version. Current development version is `1.0.1`; secure updates use the monotonic `update_sequence` independently of SemVer.
...
13. **Outcome gate** — `agent-outcome-gate.ts` + `outcome-runtime.ts`: false 「완료」 blocked unless mutate + markers on disk; also blocks overclaim, user debug deferral, weak-as-pass, HTML/JS wiring smoke fails, **wrong artifact modality**, OpenClaw bleed on local bots, invented domain APIs, missing requiredSecrets on live-publish claims.
...
15. **Outcome policy table** — `outcome-runtime.ts` (`OUTCOME_RULES`); claim helpers `agent-claim-gates.ts`; runtime smoke `agent-runtime-smoke.ts`; session mutate paths in `data/agent-run-meta/` (`agent-run-meta.ts`). **Session Exit Gate (`openGate`)** — Critic `next`/`다음 수정` persisted; run-loop injects system note; `open_gate_unclosed` blocks 완료 until evidence addresses that gate (`agent-open-gate.ts`). **Artifact contract** — `agent-artifact-contract.ts` + lockedConstraints; `verify:artifact-contract`.
...
- Outcome gate (false 완료): `core/src/agent/agent-outcome-gate.ts` (markers + structural deliverable / partial claims)
...
- Multi-agent (internal MAR): ADR-005 + `agent-mar-runtime.ts` — default on; `MY_AGENT_MULTI_AGENT=0` = single `runCodeAgent`; mutate 후 mandatory Critic(`reviewer`; `MY_AGENT_MANDATORY_CRITIC=0`으로 생략; VERDICT+JSON 재시도)
...
- **Failure plane (ADR-008):** `agent-failure-plane.ts` — infra≠assistant merge; MAR coder/planner throws; UI never demotes code→chat; mutate without workspace → policy refuse; **fail/stop persist + tool-plane 1 retry + code final-only bubble**. Verify: `verify:failure-plane`
...
- Outcome policy runtime: `core/src/agent/outcome-runtime.ts` (+ `agent-claim-gates.ts`, `agent-runtime-smoke.ts`)
